...
class FindUserSet:
    ...
    def find_users(self, controller, commit_datetime):
        ...
        self.ba_users_after = self._get_users_in_time_window(logs_time_sorted, commit_index, upper_index, "controller", controller, all_before_users)
        self.ba_users_before = self._get_users_in_time_window(logs_time_sorted, lower_index, commit_index, "controller", controller, self.ba_users_after)

        # construct the set of users who had activity only before a 
        # given commit, but not after
        self.only_before_users_before = self._get_logs_not_in_set(self.all_before_users, self.ba_users_before)
    ...
    def _add_to_dict_with_attribute(attribute_dict, activity_log, attribute, limit_attributes):
